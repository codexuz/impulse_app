<template>
  ...
  <ResultPage v-if="showResultPage" :gr_score="store.result.score" :score="Number(store.result.band)" :level="store.result.level"/>
</template>
...
const transcribeAudio = async (blob) => {
  ...
  const res = await fetch("https://edumoacademy.uz/api/transcribe", {
    method: 'POST',
    body: formData
  });
